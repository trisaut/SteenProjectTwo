...
//vampire slayer (original interactive element)
function Slayer (pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, -1.6));
  this.size = new Vector(1.7, 2.6);
  this.patrol = Math.random() * Math.PI * 2;
}
Slayer.prototype.type = "slayer";

function Grave (pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, -0.2));
  this.size = new Vector(0.73, 1.2);
  //this.patrol = Math.random() * Math.PI * 2;
}
Grave.prototype.type = "grave";

function Coffin (pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, -1.6));
  this.size = new Vector(1.4, 2.6);
  //this.patrol = Math.random() * Math.PI * 2;
}
Coffin.prototype.type = "coffin";

function Endtext (pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, -1.6));
  this.size = new Vector(1.7, 1.5);
  //this.patrol = Math.random() * Math.PI * 2;
}
Endtext.prototype.type = "endtext";
...
var wiggleSpeed = 18, wiggleDist = 0.18;

var patrolSpeed = 8, patrolDist = 0.07;
...
Slayer.prototype.act = function(step) {
  this.patrol += step * patrolSpeed;
  var patrolPos = Math.sin(this.patrol) * patrolDist;
  this.pos = this.basePos.plus(new Vector(patrolPos, 0));
};

Grave.prototype.act = function(step) {
  this.patrol += step * patrolSpeed;
  var patrolPos = Math.sin(this.patrol) * patrolDist;
  //this.pos = this.basePos.plus(new Vector(patrolPos, 0));
};

Coffin.prototype.act = function(step) {
  this.patrol += step * patrolSpeed;
  var patrolPos = Math.sin(this.patrol) * patrolDist;
  //this.pos = this.basePos.plus(new Vector(patrolPos, 0));
};

Endtext.prototype.act = function(step) {
  this.patrol += step * patrolSpeed;
  var patrolPos = Math.sin(this.patrol) * patrolDist;
  //this.pos = this.basePos.plus(new Vector(patrolPos, 0));
};

Info.prototype.act = function(step) {
  this.patrol += step * patrolSpeed;
  var patrolPos = Math.sin(this.patrol) * patrolDist;
  //this.pos = this.basePos.plus(new Vector(patrolPos, 0));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "slayer" && this.status == null) {
  this.status = "lost";
